...
export default (db: Database) =>
  db.defineSchema<ProgramSchema>('Program', {
    _id: {
      type: RDBType.STRING,
      primaryKey: true,
    },
    ...
    modules: {
      ...
      virtual: {
        name: 'Module',
        where: (moduleTable: lf.schema.Table) => ({
    ...
    ['@@dispose']: (rootEntities, scope) => {
      ...
      return matcher1({ parentId: { $in: rootEntities.map((e) => e._id) } }).pipe(
        tap(disposer1),
        concatMap((modules) => {
          const engineers = rootEntities.map((entity) => entity.ownerId).concat(modules.map((m: any) => m.ownerId))
