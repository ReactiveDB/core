...
import { subscribeOn, tap, take, concatMap, skip, toArray, catchError, finalize } from 'rxjs/operators'
...
export default describe('Database Testcase: ', () => {
  ...
  describe('Method: connect', () => {
    ...
    it('should store [[schemas]]', () => {
      ...
      expect(new Set(schema.columns.keys())).to.deep.equal(
        new Set([
          '_creatorId',
          '_executorId',
          '_projectId',
  ...
  describe('Method: get', () => {
    ...
    const getId = (tasks: TaskSchema[]) => tasks.map((t) => t._id).sort()
    ...
    it('should get value when multi clause were specified', function*() {
      const [{ _id, _projectId }] = yield database
        .get<TaskSchema>('Task', {
          where: {
            _id: target._id,
    ...
    it('should get records with order', function*() {
      ...
      fixture
        .sort((x, y) => {
          ...
          const earlier = -Math.sign(new Date(x.created).valueOf() - new Date(y.created).valueOf())
  ...
  describe('Method: update', () => {
    ...
    it('should be able to update record based on pk clause', function*() {
      ...
      yield database.update<TaskSchema>(
        'Task',
        {
          _id: target._id,
        },
  ...
  describe('case: Relational Scenario', () => {
    ...
    it('should be able to handle circular reference in imlicit mode', function*() {
      ...
      const [engineerRet] = yield database
        .get('Engineer', {
          where: {
            _id: program.ownerId,
          },
    ...
    it('should be able to handle circular reference in explicit mode', function*() {
      const [programRet] = yield database
        .get('Program', {
          fields: [
            '_id',
            'name',
  ...
  describe('Method: transaction', () => {
    ...
    it('should able to create a transaction scope for multi query [6]', function*() {
      ...
      const ret = yield database.transaction().pipe(
        concatMap(([db, tx]) => {
          return db.remove('Program').pipe(concatMap(() => tx.commit()))
        }),
      )
