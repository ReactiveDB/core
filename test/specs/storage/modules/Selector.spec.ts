...
export default describe('Selector test', () => {
  ...
  describe('Selector.prototype.setMapFn', () => {
    it('setMapFn should work fine with values', function* () {
      const selector = new Selector<Fixture>(db,
        db.select().from(table),
        tableShape,
        new PredicateProvider(table, { time: { $gte: 50 } })
      ...
      selector.setMapFn((_) => 1)

      const results = yield selector.values()

      expect(results).to.have.lengthOf(1000 - 50)
      results.forEach((ret: any) => expect(ret).to.equal(1))
    })

    it('setMapFn should work fine with changes', function* () {
      const selector = new Selector<Fixture>(db,
        db.select().from(table),
        tableShape,
        new PredicateProvider(table, { time: { $gte: 50 } })
      ...
      const results = yield selector.changes().take(1)

      expect(results).to.have.lengthOf(1000 - 50)
      results.forEach((ret: any) => expect(ret).to.equal(1))
    })

    it('setMapFn should work fine with combine and changes', function* () {
      const selector1: Selector<any> = new Selector(db,
        db.select().from(table),
        tableShape,
        new PredicateProvider(table, { time: { $lt: 50 } })
      )
      const selector2: Selector<any> = new Selector(db,
        db.select().from(table),
        tableShape,
        new PredicateProvider(table, {
          time: {
      ...
      selector1.setMapFn((_) => 1)

      const distSelector = selector1.combine(selector2)

      yield distSelector.changes()
        .take(1)
        .do(r => {
          r.slice(0, 50).forEach(v => expect(v).to.equal(1))
        })
    })

    it('setMapFn should work fine with combine and values', function* () {
      const selector1: Selector<any> = new Selector(db,
        db.select().from(table),
        tableShape,
        new PredicateProvider(table, { time: { $lt: 50 } })
    ...
    it('setMapFn should work fine with concat and changes', function* () {
      const selector1: Selector<any> = new Selector(db,
        db.select().from(table),
        tableShape,
        new PredicateProvider(table, { time: { $lt: 50 } })
