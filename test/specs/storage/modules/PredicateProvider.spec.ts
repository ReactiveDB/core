...
import { PredicateProvider, lfFactory, DataStoreType, Predicate, Logger } from '../../../index'

use(SinonChai)

export default describe('PredicateProvider Testcase: ', () => {

  describe('Class: PredicateProvider', () => {
    const dataLength = 1000
      ...
      describe('Static Method: parse', () => {
        ...
        it('should parse normal predicate', () => {
          ...
          const result = PredicateProvider.parse(meta)
      ...
      describe('Static Method: check', () => {
        it('should be able to check if input is valid', () => {
          expect(!!PredicateProvider.check(null)).to.equal(false)
          expect(!!PredicateProvider.check(false)).to.equal(false)
          expect(!!PredicateProvider.check([] as any)).to.equal(false)
          expect(!!PredicateProvider.check({ $match: /\d/ })).to.equal(true)
          expect(!!PredicateProvider.check({ $and: [] })).to.equal(true)
        })
      })

      describe('Static Method: checkNull', () => {
        let spy: any = null
        ...
        it('should warn if failed to build predicate', () => {
          PredicateProvider.checkNull(null, 'Foo')
